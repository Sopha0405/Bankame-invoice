...
from app.invoice_quality_rules import RESPONSE_SCHEMA, build_prompt
...
class InvoiceQualityUpstreamError(Exception):
...
def parse_response(response: Any, model_name: str, request_id: str) -> GeminiCallResult:
    ...
    if not candidates:
        logger.warning(
            "invoice_quality gemini_sin_candidates request_id=%s model=%s block_reason=%s",
            request_id,
            model_name,
            block_reason,
        )
        raise InvoiceQualityUpstreamError(
            CODE_UNAVAILABLE,
    ...
    if finish_reason is not None and finish_reason not in NORMAL_FINISH_REASONS:
        logger.warning(
            "invoice_quality gemini_finish_reason_anomalo request_id=%s model=%s "
    ...
    if not raw_text or not raw_text.strip():
        logger.warning(
            "invoice_quality gemini_respuesta_vacia request_id=%s model=%s finish_reason=%s",
            request_id,
            model_name,
            finish_reason,
        )
    ...
    try:
        ...
        # No se loguea raw_text: puede traer datos de la factura.
        logger.warning(
            "invoice_quality gemini_json_invalido request_id=%s model=%s "
    ...
    if not isinstance(payload, dict):
        logger.warning(
            "invoice_quality gemini_json_no_objeto request_id=%s model=%s tipo=%s",
            request_id,
            model_name,
...
async def request_verdict(
    ...
    contents = [
        build_prompt(kind),
        # Parte inline: el SDK hace el base64. Hasta 10 MB no hace falta File API.
        types.Part.from_bytes(data=file_bytes, mime_type=mime_type),
    ]

    models = attempt_plan()
    hard_timeout = TIMEOUT_MS / 1000 + TIMEOUT_MARGIN_SECONDS
    last_error: Optional[InvoiceQualityUpstreamError] = None
    ...
    for attempt, model_name in enumerate(models, start=1):
        try:
        ...
        except InvoiceQualityUpstreamError:
            # Respuesta impaseable o bloqueada: no se reintenta.
            raise

        except (asyncio.TimeoutError, TimeoutError):
            logger.warning(
                "invoice_quality gemini_timeout request_id=%s model=%s intento=%s "
                "timeout_ms=%s",
                request_id,
                model_name,
                attempt,
            ...
            last_error = InvoiceQualityUpstreamError(
                CODE_UNAVAILABLE,
            ...
            logger.warning(
                "invoice_quality gemini_client_error request_id=%s model=%s intento=%s "
            ...
            if status_code == 429:
                ...
                logger.error(
                    "invoice_quality gemini_no_reintentable request_id=%s status_code=%s",
                    request_id,
                    status_code,
                )
                raise InvoiceQualityUpstreamError(
            ...
            logger.warning(
                "invoice_quality gemini_server_error request_id=%s model=%s intento=%s "
                "status_code=%s status=%s",
                request_id,
                model_name,
                attempt,
            ...
            logger.warning(
                "invoice_quality gemini_error_inesperado request_id=%s model=%s "
    ...
    raise last_error or InvoiceQualityUpstreamError(
        CODE_UNAVAILABLE,
        "No se pudo consultar a Gemini.",
        503,
    )
